...
def inference(model=None):
    ...
    main_title_cfg = """<div><h1 style="color:#FF64DA; text-align:center; font-size:40px; 
                    ...
                    Kmphitech YOLO Streamlit Application
                    </h1></div>"""

    # Subtitle of streamlit application
    sub_title_cfg = """<div><h4 style="color:#042AFF; text-align:center; 
                    font-family: 'Archivo', sans-serif; margin-top:-15px; margin-bottom:50px;">
                    Experience real-time object detection on your webcam with the power of YOLO! 🚀</h4>
                    </div>"""

    # Set html page configuration
    st.set_page_config(
        page_title="Kmphitech AIML Demo App",
        layout="wide",
    ...
    if model:
        available_models.insert(
            0, model.split(".pt")[0]
        )  # insert model without suffix as *.pt is added later
    ...
    if st.sidebar.button("Start"):
        ...
        while videocapture.isOpened():
            ...
            if enable_trk == "Yes":
                results = model.track(
                    frame, conf=conf, iou=iou, classes=selected_ind, persist=True
                )
            else:
